...
void clearLEDs() {
  // loop through all spokes and clear all LEDs
  for (uint16_t i = 0; i < NUM_SPOKES; i++) {
  ...
  if (wifly.isConnected() == false) {
    ...
    if (available > 0) {
      ...
      if (ch == '\n') {
        ...
        //        r = str.substring(3, 6).toInt();
        //        g = str.substring(7, 10).toInt();
        //        b = str.substring(11, 14).toInt();
